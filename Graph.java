...
import java.util.ArrayList;
 ...
 public class Graph {
   ...
   public int findRoot() {
     // If directed, check in degree of node (amount of edges going into the node)
     // All nodes with in degree of 0 are root nodes
     // If undirected root node is arbitrary (first node at index 0?)
     // create linked list 0-num, as find which index are in dest, 
     // and remove from linked list
      ...
      int i = 0;
       while (i < numVertices) {   
            if (adjListArr[i] != null) {
               LinkedList<Integer> val = adjListArr[i];   
               for(Integer dest : val){
                 indexes.remove(Integer.valueOf(dest));
               }
           }
           i++;
       }
       if(indexes.size() == 1){
        return vertexValues.get(indexes.get(0));
       }else{
        return -1;
       }
