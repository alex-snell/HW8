 ...
 class ProblemSolutions {
     ...
     public boolean canFinish(int numExams, 
         ...
         ArrayList<Integer>[] adj = getAdjList(numExams, 
                                         prerequisites); 
         //Create a visited array to place visited nodes
     ...
     //helper method
     private boolean hasCycle(ArrayList<Integer>[] adj, int node, int[] visited) {
         visited[node] = 1; // Mark as "currently visiting"
         for (int prereq : adj[node]) {
             if (visited[prereq] == 0) {
                 if (hasCycle(adj, prereq, visited)) {
                     return true; //recursively check for cycle
                 }
             } else if (visited[prereq] == 1) {
                 return true; 
             }
         }
         visited[node] = 2; // Mark as "fully visited" after exploring all neighbors
         return false;
     }
    ...
    //Keep track of number of seperate groups
    int groupCount = 0;
    for(i = 0; i < numNodes; i++) {
        if(!visited[i]) {
            groupCount++;
    ...
    private void dfs(int node, Map<Integer, List<Integer>> graph, boolean[] visited) {
        visited[node] = true; //node now visited
        //Handles null pointer exceptions, returns List of neighbors
        List<Integer> neighbors = graph.getOrDefault(node, new ArrayList<>()); 
        for (int neighbor : neighbors) {
            if (!visited[neighbor]) {
                dfs(neighbor, graph, visited); //recursievly search to get visited
            }
        }
    }
 }
